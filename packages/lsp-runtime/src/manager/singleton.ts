 ...
 *   - reinitialize 支持（plugin 刷新后调用，SnSworker 等价：动态配置变更）
 ...
 *
 *   SnSworker 没有 bare 模式但需要等价的 escape hatch（如 daemon 单次 RPC 不需要 LSP）。
 *   采用环境变量 + 参数双控制是 SnSworker 自定义但语义等价的扩展。
 */
 ...
 *
 * SnSworker 扩展点（本仓库扩展）：C5 阶段 `registerLSPNotificationHandlers`
 ...
 *
 * This function is called during SnSworker startup. It synchronously creates
...
export function initializeLspServerManager(
  ...
  // SnSworker disabled 模式（bare mode）：跳过初始化
