...
function checkFilePathSecurity(
  ...
  if (isWrite && workspaceRoots.length > 0 && !inWorkspace) {
    ...
    // 但这些产品名一旦 UI 改名就漂移；且 LLM 不一定理解 SnSworker 的 i18n
 ...
 *
 * **Why no Tier 3 (block-anchor)**: SnSworker originally had a "first-line + last-line anchored,
 ...
 * | `line_trimmed`  | 整行 trim 后命中（SnSworker 特有兜底，多一层）    |
 */
type MatchResult = {
  start: number;
  end: number;
 ...
 * Flexible / ContextAware（「首末锚定中间不校验」，SnSworker Wave 1
 * dogfood 已验证会假阳性命中）。
 */
...
// - 生产用 `--sortr=modified` 保持 SnSworker 既有"最新在前"承诺；测试环境用 `--sort=path` 稳定排序
async function globSearch(pattern: string, cwd: string, includeIgnored = false): Promise<string[]> {
  ...
  const args = [
    '--files',
    // 部分实现用 `--sort=modified`，但 ripgrep 该方向是旧文件在前；SnSworker
...
export const codeGrepTool: AgentTool<CodeGrepInput, CodeGrepOutput> = {
  ...
  async execute(input: CodeGrepInput): Promise<CodeGrepOutput> {
    ...
    try {
      ...
      // `.github/workflows/*` / `.env*` 等隐藏目录的配置——SnSworker 之前不传 `--hidden`
