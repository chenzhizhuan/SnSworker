 ...
 * 转成 `X-SnSworker-Context-Tier` 透给 Django 代理，Django 侧再把档位的
...
describe('TabTinProxyProvider context tier header', () => {
  it('静态 contextTierId 注入 X-SnSworker-Context-Tier', () => {
    ...
    expect(headers['X-SnSworker-Context-Tier']).toBe('long_1m');
  });
  ...
  it('函数型 contextTierId 每次取值（支持 session 切档后立即生效）', () => {
    ...
    expect(headersOf(provider)['X-SnSworker-Context-Tier']).toBe('standard');

    current = 'long_1m';
    expect(headersOf(provider)['X-SnSworker-Context-Tier']).toBe('long_1m');

    current = undefined;
    expect(headersOf(provider)['X-SnSworker-Context-Tier']).toBeUndefined();
  });
  ...
  it('用逻辑 LLM 调用键派生 attempt 计费键', () => {
    ...
    expect(headers['X-SnSworker-Billing-Logical-Key']).toBe('agent-turn:job-1:_main_chat:0');
    expect(headers['X-SnSworker-Billing-Idempotency-Key']).toBe(
      'agent-turn:job-1:_main_chat:0:attempt:0',
    );
    expect(headers['X-SnSworker-Billing-Attempt-Key']).toBe(
      'agent-turn:job-1:_main_chat:0:attempt:0',
    );
