 ...
 *   **拒绝** streak（不是工具成功复读）。
 * - **SnSworker tool-failure-tracker**：本 tracker 的 SSoT 来源。状态机、stage
 ...
 * 名而非 Agent，避免"问责"姿态；产品主语用 SnSworker。
 *
...
export function buildToolRepetitionNoticeContent(
  ...
  return (
    ...
    'SnSworker 正在关注，再重复几次会主动提示 Agent 别再重发。' +
...
export function buildToolRepetitionNudgeContent(
  ...
  return (
    `工具「${trigger.tool}」在 ${seconds} 秒内被相同输入调用了 ${trigger.count} 次。` +
    'SnSworker 已主动提示 Agent 别再用相同输入重发，请稍候——' +
