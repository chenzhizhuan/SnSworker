/**
 * SnSworker 权威 tool_use id 映射。
 *
 * 上游模型（如 Kimi）常返回 `{tool_name}_{n}`，会话内会回绕撞号；
 * fork 若原样拷贝这些 id，子会话会与父系命名空间叠在一起。
 *
 * 契约：
 * - 持久层 / state.messages / HITL / transcript **只存** SnSworker id（`tu_<uuid>`）
