...
function buildPersonalPluginRuntimeInstruction(skill: SkillRecord, key: string): string | null {
  ...
  return [
    ...
    '',
    '必须直接调用 `run_terminal_command` 执行下面的 SnSworker CLI 命令：',
    '',
    '```bash',
    command,
    '```',
    ...
    '插件安装目录、local service cwd、项目目录、MCP attach 和浏览器打开都由 SnSworker CLI 转交宿主授权流程，并根据 Personal Plugin registry 自动处理。',
...
function buildSkillInvokeContextModifier(
  ...
  if (meta.model) {
    ...
    if (isValid) {
      ...
      // 能直接理解回退原因 + 怎么修。SnSworker 面向中文用户，英文系统
      // 提示容易让 LLM 切换到英文回复。
