...
describe('checkHardlinePath · 绝对路径红线', () => {
  ...
  it('Windows device / 管理员共享 / 系统环境变量归一后命中', () => {
    ...
    expect(checkHardlinePath('$env:ProgramFiles\\SnSworker', 'shell').hit).toBe(true);
    expect(checkHardlinePath('$env:ProgramW6432\\SnSworker', 'shell').hit).toBe(true);
    expect(checkHardlinePath('${env:ProgramFiles(x86)}\\SnSworker', 'shell').hit).toBe(true);
...
describe('extractPathsFromCommand · 跨平台 shell 路径', () => {
  ...
  it('提取 Windows 盘符路径与含空格引号路径', () => {
    expect(extractPathsFromCommand(
      'Remove-Item -LiteralPath "C:\\Program Files\\SnSworker\\app.exe" D:/outside/file.txt',
    )).toEqual([
      'C:\\Program Files\\SnSworker\\app.exe',
      'D:/outside/file.txt',
    ]);
  });
