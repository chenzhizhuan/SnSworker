...
default_prompt: "Use the asset-gen tools to generate images, 3D models, or animated sprites for this game."
...
Generate PNG images (Gemini or xAI Grok) and GLB 3D models (Tripo3D) from text prompts. These are paid APIs — every call costs real money. Tools live at `C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/`; run from the project root and keep runtime-loaded outputs under `assets/`.
...
```bash
python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py image \
...
Read `C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/rembg.md`. Key rule: **never prompt for a "transparent background"** (the generator bakes a checkerboard) — prompt a solid color, then matte it out.
...
```bash
python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py glb  --image ref.png -o model.glb     # 30¢ default / 60¢ --quality hd
python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py rig  --image ref.png -o rigged.glb    # +25¢, biped only
python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py retarget --rigged rigged.glb \
...
- A timeout in `glb`/`rig`/`retarget` does **not** mean server failure. The task id is already saved in the `<output>.tripo.json` sidecar. **Do not resubmit — that double-charges.** Resume for free instead:
  ```bash
  python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py resume -o model.glb
  ```
...
Each command prints JSON to stdout: `{"ok": true, "path": "...", "cost_cents": 7}`. Progress goes to stderr. On TeleAgent (Windows/PowerShell), run directly and read stdout JSON; if it fails, the error is also printed to stdout as `{"ok": false, "error": "..."}`:

```powershell
python C:/Users/meepo/.config/TeleAgent/skills/godogen/asset-gen/tools/asset_gen.py image --prompt "..." -o p.png
```

Generate independent images in parallel (issue multiple powershell tool calls in one message).
