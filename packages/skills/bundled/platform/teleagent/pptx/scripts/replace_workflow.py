...
Usage:
    python replace_workflow.py <input.pptx> <replacements.json> <output.pptx>

The replacements JSON should have the structure output by inventory_workflow.py.
ALL text shapes identified by inventory_workflow.py will have their text cleared
...
from inventory_workflow import InventoryData, extract_text_inventory_impl
...
def run_cli():
    ...
    try:
        apply_replacements_impl(str(input_pptx), str(replacements_json), str(output_pptx))
...
def apply_replacements_impl(pptx_file: str, json_file: str, output_file: str):
    ...
    inventory = extract_text_inventory_impl(Path(pptx_file), prs)

    # Detect text overflow in original presentation
    original_overflow = detect_frame_overflow_impl(inventory)

    # Load replacement data with duplicate key detection
    with open(json_file, "r") as f:
        replacements = json.load(f, object_pairs_hook=check_duplicate_keys_impl)

    # Validate replacements
    errors = validate_replacements_impl(inventory, replacements)
    if errors:
        ...
        print(
            "You can regenerate the inventory with: python inventory_workflow.py <input.pptx> <output.json>"
        )
    ...
    for slide_key, shapes_dict in inventory.items():
        ...
        for shape_key, shape_data in shapes_dict.items():
            ...
            for i, para_data in enumerate(replacement_shape_data["paragraphs"]):
                ...
                apply_paragraph_properties_impl(p, para_data)
    ...
    try:
        updated_inventory = extract_text_inventory_impl(tmp_path)
        updated_overflow = detect_frame_overflow_impl(updated_inventory)
    finally:
...
def check_duplicate_keys_impl(pairs):
...
def validate_replacements_impl(inventory: InventoryData, replacements: Dict) -> List[str]:
...
def detect_frame_overflow_impl(inventory: InventoryData) -> Dict[str, Dict[str, float]]:
...
def apply_font_properties_impl(run, para_data: Dict[str, Any]):
...
def apply_paragraph_properties_impl(paragraph, para_data: Dict[str, Any]):
    """Apply formatting properties to a paragraph."""
    # Get the text but don't set it on paragraph directly yet
    text = para_data.get("text", "")

    # Get or create paragraph properties
    pPr = clear_paragraph_bullets_impl(paragraph)
    ...
    apply_font_properties_impl(run, para_data)

def clear_paragraph_bullets_impl(paragraph):
