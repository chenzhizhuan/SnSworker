...
Usage:
    python rearrange_workflow.py template.pptx output.pptx 0,34,34,50,52
...
def rearrange_presentation_impl(template_path, output_path, slide_sequence):
    """
    ...
    for i, template_idx in enumerate(slide_sequence):
        if template_idx in duplicated and duplicated[template_idx]:
            ...
            for _ in range(count):
                duplicate_slide_impl(prs, template_idx)
    ...
    for target_pos in range(len(slide_map)):
        ...
        if current_pos != target_pos:
            reorder_slides_impl(prs, current_pos, target_pos)
...
def reorder_slides_impl(pres, slide_index, target_index):
...
Examples:
  ...
  python rearrange_workflow.py template.pptx output.pptx 5,3,1,2,4
...
Note: Slide indices are 0-based (first slide is 0, second is 1, etc.)
    ...
    try:
        rearrange_presentation_impl(template_path, output_path, slide_sequence)
