...
Main Functions:
    extract_text_inventory_impl: Extract all text from a presentation
    save_inventory: Save extracted data to JSON

Usage:
    python inventory_workflow.py input.pptx output.json
"""
...
def save_inventory_impl(inventory: InventoryData, output_path: Path) -> None:
...
def get_inventory_as_dict_impl(pptx_path: Path, issues_only: bool = False) -> InventoryDict:
    """Extract text inventory and return as JSON-serializable dictionaries.

    This is a convenience wrapper around extract_text_inventory_impl that returns
    ...
    """
    inventory = extract_text_inventory_impl(pptx_path, issues_only=issues_only)
...
def extract_text_inventory_impl(
    ...
    for slide_idx, slide in enumerate(prs.slides):
        ...
        for shape in slide.shapes:  # type: ignore
            shapes_with_positions.extend(collect_shapes_with_absolute_positions_impl(shape))
        ...
        sorted_shapes = sort_shapes_by_position_impl(shape_data_list)
        ...
        if len(sorted_shapes) > 1:
            detect_overlaps_impl(sorted_shapes)
...
def detect_overlaps_impl(shapes: List[shape_data_impl]) -> None:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            overlaps, overlap_area = calculate_overlap_impl(rect1, rect2)
...
def sort_shapes_by_position_impl(shapes: List[shape_data_impl]) -> List[shape_data_impl]:
...
def collect_shapes_with_absolute_positions_impl(
    shape: BaseShape, parent_left: int = 0, parent_top: int = 0
) -> List[shape_with_position_impl]:
    ...
    if hasattr(shape, "shapes"):  # GroupShape
        ...
        for child in shape.shapes:  # type: ignore
            result.extend(
                collect_shapes_with_absolute_positions_impl(
...
def is_valid_shape_impl(shape: BaseShape) -> bool:
...
class shape_data_impl:
    ...
    @property
    def paragraphs(self) -> List[paragraph_data_impl]:
        ...
        for paragraph in self.shape.text_frame.paragraphs:  # type: ignore
            if paragraph.text.strip():
                paragraphs.append(paragraph_data_impl(paragraph))
        return paragraphs
    ...
    def _estimate_frame_overflow(self) -> None:
        ...
        for para_idx, paragraph in enumerate(text_frame.paragraphs):
            ...
            para_data = paragraph_data_impl(paragraph)
...
@dataclass
class shape_with_position_impl:
...
def run_cli():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python inventory_workflow.py presentation.pptx inventory.json
    Extracts text inventory with correct absolute positions for grouped shapes

  python inventory_workflow.py presentation.pptx inventory.json --issues-only
...
The output JSON includes:
  ...
  - Issue detection: text overflow and shape overlaps
    ...
    try:
        ...
        inventory = extract_text_inventory_impl(input_path, issues_only=args.issues_only)

        output_path = Path(args.output)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        save_inventory_impl(inventory, output_path)
