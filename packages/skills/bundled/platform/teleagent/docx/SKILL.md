...
2. **转换为 JS**：使用下方列出的 CLI 参数运行 `md_to_js.py`。**不要读取脚本源码**——下方的参数表包含了使用该脚本所需的全部信息。只有在调试或修复脚本 bug 时才需要读取 `md_to_js.py` 源码。
   ```powershell
   $env:PYTHONUTF8="1"; python scripts/md_to_js.py --input <content.md> --output <output.js> --docx-output <output.docx> --font-size 24 --indent 480 --line-spacing 360
   ```

   | 参数 | 默认值 | 说明 |
   |------|--------|------|
   | `--font` | 按平台选择 | 文档默认字体；macOS 使用 PingFang SC，Windows 使用 Microsoft YaHei，Linux 使用 Noto Sans CJK SC；显式传值可覆盖 |
   ...
   - 默认字体会按生成平台自动选择；仅在用户或格式规范明确指定时传入 `--font "字体名"`
   - **封面页**：需要封面时传入 `--cover --subtitle "XX省全景介绍" --tagline "关键词 · 关键词 · 关键词"`
   - 使用 `--cover` 时封面独立为第一节（无页眉页脚），若有 TOC 则第二节为目录（无页脚），正文为最后一节，页码从 1 开始
   - 封面日期自动取当前年月，无需传入
