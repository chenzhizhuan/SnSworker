...
def default_font(system=None):
    """Return a practical CJK default for the current generation platform."""
    return DEFAULT_CJK_FONTS.get(system or platform.system(), "Arial")
...
def generate_js(blocks, output_path, font=None, input_dir=None,
                ...
                cover=False, no_header=False, no_footer=False):
    """生成完整的 docx-js JS 文件"""
    ...
    font_name = (font or default_font()).replace('"', '\\"')
    ...
    children_parts = []
    cover_parts = []  # 封面内容，单独进 section（无页眉页脚）
    toc_parts = []    # TOC 内容，cover 模式下单独进 section（无页脚）
    ...
    # A4 纸宽 11906 DXA，可用宽度扣除左右边距（表格/图片使用）
    page_content_width = 11906 - margin_left - margin_right
    ...
    for block in blocks:
        ...
        if btype == "heading":
            ...
            pbb = "        pageBreakBefore: true,\n" if page_break_next else ""
            page_break_next = False
            img_width = int(page_content_width * 0.8)
            children_parts.append(
                f'      new Paragraph({{\n'
                f'{pbb}'
                f'        alignment: AlignmentType.CENTER,\n'
                ...
                f'          transformation: {{ width: {img_width} }},\n'
    ...
    if cover_parts:
        ...
        if toc_parts:
            sections_js = (
                "  sections: [\n"
                "    {\n"
               f"      properties: {{ page: {{ {section_props} }} }},\n"
                "      children: [\n"
                + cover_children_str + "\n"
                "      ]\n"
                "    },\n"
               ...
               f"      properties: {{ page: {{ {section_props} }} }},\n"
                ...
                "    },\n"
                "    {\n"
               f"      properties: {{ page: {{ {section_props}, pageNumbers: {{ start: 1 }} }} }},\n"
                + header_section_js +
                footer_section_js +
                "      children: [\n"
                + children_str + "\n"
...
def main():
    ...
    parser.add_argument(
        "--font",
        default=None,
        help="文档默认字体（默认按平台选择：macOS=PingFang SC，Windows=Microsoft YaHei，Linux=Noto Sans CJK SC）",
    )
    ...
    # 过滤占位符：若传入了尖括号占位符（如 <副标题>、<关键词>），视为未传入
