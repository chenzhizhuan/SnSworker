...
def main():
    ...
    if args.image:
        # 图片消息（可同时附带文字说明）
        result = send_image_via_db(args.image, text=args.text, timeout=args.timeout)
    elif args.text:
        # 纯文本消息
        result = send_message_via_db(args.text, timeout=args.timeout)
    else:
        parser.error("需要指定 --text 或 --image 参数")
