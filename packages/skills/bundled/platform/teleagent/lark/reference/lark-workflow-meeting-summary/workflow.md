...
> **Version**: `1.0.0`
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../lark-shared/workflow.md`](../lark-shared/workflow.md)，其中包含认证、权限处理**。然后阅读 [`../lark-vc/workflow.md`](../lark-vc/workflow.md)，了解会议纪要相关操作。
...
> - `unified`：逐字稿**不是独立文档**，没有可分享的逐字稿文档链接；需要逐字稿内容时用 `note +transcript --note-id <note_id>`（[lark-note](../lark-note/workflow.md)）拉取到本地，报告中标注"unified 纪要"即可。
...
阅读 [`../lark-doc/workflow.md`](../lark-doc/workflow.md) 学习云文档技能。
...
- [lark-shared](../lark-shared/workflow.md) — 认证、权限（必读）
- [lark-vc](../lark-vc/workflow.md) — `+search`、`+notes` 详细用法
- [lark-note](../lark-note/workflow.md) — `note +detail`、`note +transcript`（unified 纪要逐字稿）
- [lark-doc](../lark-doc/workflow.md) — `+fetch`、`+create`、`+update` 详细用法