...
详细用法请参考 [lark-doc](../../lark-doc/workflow.md) 与 [lark-note](../../lark-note/workflow.md) skill。
...
- 已知 `note_id` 后切到 [lark-note](../../lark-note/workflow.md)；逐字稿路由以 `lark-note` 的 `note_display_type` 规则为准。
...
- **会议产物的文档本质**：智能纪要（`note_doc_token`）和 `normal` 纪要的逐字稿（`verbatim_doc_token`）都是飞书文档，需要通过 `lark-doc` 的 API（如 `docs +fetch --api-version v2`）查询其内容和元信息；`unified` 纪要的逐字稿不是独立文档，用 `note +transcript` 拉取（[lark-note](../../lark-note/workflow.md)）。
