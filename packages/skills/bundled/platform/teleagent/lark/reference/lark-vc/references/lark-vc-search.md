...
> **前置条件：** 先阅读 [`../lark-shared/workflow.md`](../../lark-shared/workflow.md) 了解认证、全局参数和安全规则。
...
`vc +search` 只能搜索已结束的历史会议记录，不用于查询未来日程。查询未来会议安排请使用 [lark-calendar](../../lark-calendar/workflow.md)。
...
| 搜不到未来会议 | `vc +search` 只查历史会议 | 改用 [lark-calendar](../../lark-calendar/workflow.md) 查询未来日程 |
