...
> **Version**: `1.0.0`
> **CLI Help**: `lark-cli vc --help`
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../lark-shared/workflow.md`](../lark-shared/workflow.md)，其中包含认证、权限处理**
...
| 查日历/日程或未来时间的会议 | [lark-calendar](../lark-calendar/workflow.md) |
| 查"今天有哪些会议" | `vc +search`（已结束）+ lark-calendar（未开始），合并展示 |
| 只按自然语言标题查"xx 纪要的逐字稿 / 原始记录 / 谁说了什么" | 先到 [lark-drive](../lark-drive/workflow.md) / [lark-doc](../lark-doc/workflow.md)；仅在已拿到 `note_id` / `vc-node-id` 后再到 [lark-note](../lark-note/workflow.md) |
| Agent 真实入会/离会、会中实时事件 | [lark-vc-agent](../lark-vc-agent/workflow.md) |
| 本地音视频文件转纪要/逐字稿 | 先走 [lark-minutes](../lark-minutes/workflow.md) 上传，再回 `vc +notes --minute-tokens` |

## 核心概念

- **视频会议（Meeting）**：飞书视频会议实例，通过 meeting_id 标识。已结束的会议支持通过关键词、时间段、参会人、组织者、会议室等条件搜索（见 `+search`）。
- **会议纪要（Note）**：视频会议结束后生成的结构化文档，通过 `note_id` 标识，包含纪要文档（总结、待办）和逐字稿文档。`note_display_type` 区分**普通纪要（`normal`）**和 **unified 纪要**；已知 `note_id` 的直查与 unified 原始记录请用 [lark-note](../lark-note/workflow.md)。
...
> **逐字稿路由**：先看 `vc +notes` 返回的 `note_display_type`，不要只看 `verbatim_doc_token` 是否为空。具体路由以 [`+notes`](references/lark-vc-notes.md) 和 [lark-note](../lark-note/workflow.md) 为准。
>
...
| 已结束会议的发言内容 | 先 `vc +notes`，再按 `note_display_type` 路由 | 本 skill / [`lark-note`](../lark-note/workflow.md) |
| **进行中会议**的实时事件流（转写、聊天、共享、会中加入/离开）| `vc +meeting-events` | [`lark-vc-agent`](../lark-vc-agent/workflow.md) |
| **Agent 真实入会 / 离会** | `vc +meeting-join` / `vc +meeting-leave` | [`lark-vc-agent`](../lark-vc-agent/workflow.md) |
...
> **妙记边界**：`+notes` 负责纪要内容、逐字稿和 AI 产物；妙记基础信息请优先看 [`+recording`](references/lark-vc-recording.md) 与 [lark-minutes](../lark-minutes/workflow.md)。
>
> **Note 域边界**：`vc +notes` 是从**会议线索**（`meeting_id` / `calendar_event_id` / `minute_token`）定位纪要的入口，返回 `note_id` 和 `note_display_type`。
> - 已有 `note_id` → [lark-note](../lark-note/workflow.md)。
> - 已有 `doc_token` 且目标是读正文 → [lark-doc](../lark-doc/workflow.md)。
> - 只有自然语言纪要标题 → 文档搜索 / Docx 正文读取；有显式 `vc-node-id` 才进入 [lark-note](../lark-note/workflow.md)。
...
### minutes（跨域，详见 [lark-minutes](../lark-minutes/workflow.md)）

  - `get` — 获取妙记基础信息（标题、时长、封面）；查询妙记**内容**请用 `+notes --minute-tokens <minute-token>`

## 不在本 skill 范围

- 查询未来的会议日程 → [lark-calendar](../lark-calendar/workflow.md)
- Agent 真实入会/离会、会中实时事件 → [lark-vc-agent](../lark-vc-agent/workflow.md)
- 只有纪要文档标题的逐字稿查询 → 文档搜索 / Docx 正文读取；有显式 `vc-node-id` 才进入 [lark-note](../lark-note/workflow.md)
- 本地音视频文件转纪要/逐字稿 → [lark-minutes](../lark-minutes/workflow.md)（上传后回 `vc +notes`）
- 妙记搜索/下载/上传/重命名/替换说话人 → [lark-minutes](../lark-minutes/workflow.md)
