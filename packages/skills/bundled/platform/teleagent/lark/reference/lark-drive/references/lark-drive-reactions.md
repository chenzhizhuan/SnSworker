...
> **前置条件：** 先阅读 [`../workflow.md`](../workflow.md) 了解 Drive 评论入口，再阅读 [`lark-drive-comments-guide.md`](lark-drive-comments-guide.md) 了解评论卡片模型、评论数/回复数统计口径、`file_token` / `file_type` 规则；同时阅读 [`../../lark-shared/workflow.md`](../../lark-shared/workflow.md) 了解认证、全局参数和安全规则。
...
- [lark-drive](../workflow.md) -- 云空间（云盘/云存储）全部命令
