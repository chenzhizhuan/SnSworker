...
> **Version**: `1.0.0`
> **CLI Help**: `lark-cli note --help`
身份：仅使用 `--as user`。使用前阅读 [`../lark-shared/workflow.md`](../lark-shared/workflow.md)。
...
- `note_doc_token`：纪要正文文档，正文读取交给 [lark-doc](../lark-doc/workflow.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` / `calendar_event_id` / `minute_token` 定位纪要 → [lark-vc](../lark-vc/workflow.md)。
- 自然语言纪要标题搜索 → [lark-drive](../lark-drive/workflow.md) / [lark-doc](../lark-doc/workflow.md)。
- Docx 正文读取 → [lark-doc](../lark-doc/workflow.md)。
- 妙记基础信息与媒体文件 → [lark-minutes](../lark-minutes/workflow.md)。
