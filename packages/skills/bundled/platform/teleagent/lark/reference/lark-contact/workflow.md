...
> **Version**: `1.0.0`
> **CLI Help**: `lark-cli contact --help`
...
已知 open_id 只是想发消息 / 排日程,不必经过 contact —— 直接 [`lark-im`](../lark-im/workflow.md) / [`lark-calendar`](../lark-calendar/workflow.md)。
...
- **41050 / Permission denied** 受当前身份的可见范围限制(两条命令都可能遇到)。换 bot 身份或让管理员调整可见范围,细节见 [`lark-shared`](../lark-shared/workflow.md)。
...
- 发消息 / 查聊天记录 → [`lark-im`](../lark-im/workflow.md)
- 排日程 / 邀请会议 → [`lark-calendar`](../lark-calendar/workflow.md)
- 部门树 / 按部门列员工 / 组织架构 → [`lark-openapi-explorer`](../lark-openapi-explorer/workflow.md) 查找原生接口
