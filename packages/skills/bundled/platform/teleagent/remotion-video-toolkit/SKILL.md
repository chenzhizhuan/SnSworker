...
本技能（`remotion-video-toolkit`）是 Remotion 相关技能的**主入口**，以下两个已归档技能的内容均已整合于本技能：

### remotion（已归档）
原始参考技能，专注于 Remotion 规则的逐一说明。内容已全部迁移至上方规则表格（`rules/` 目录下），使用方式完全相同。

### remotion-best-practices（已归档）
最佳实践技能，内容与 `remotion-video-toolkit` 完全重叠。所有最佳实践规则均已收录于 `rules/` 目录，无需单独引用。

**使用方式**：直接使用本技能，通过上方规则表格中的链接跳转到具体规则文件即可。
