 ...
 *
 * **解决什么问题**：v1.8 之前 SnSworker 全仓没有"App configSchema → runtime
 ...
 *   降级到调用方 defaults，不抛错（不让 SnSworker 启动被一个配置文件错误卡死）。
 */
 ...
 *
 * 三条路径都要存在性兜底——SnSworker 客户端、Daemon、CI、单测可能从不同
 * cwd 启动。
 */
/**
 ...
 *
 * 错误兜底：所有来源失败都不抛错，最终返回 `defaults` ——避免 SnSworker 启动
 * 被一个 App 的配置文件错误卡死。
 *
