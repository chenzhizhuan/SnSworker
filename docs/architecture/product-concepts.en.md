...
SnSworker aims to reduce the coordination cost of people and Agents working together, so individual work can be continued, reused, and reviewed by the team.
...
SnSworker currently provides two complementary forms of work handoff:

- **Task continuation**: the sender transfers an Agent task to another member of the same Organization. SnSworker freezes the necessary shareable conversation context and records documents, tables, cloud files, and local files referenced by the task. After the recipient selects their own Agent and Workspace, SnSworker creates an independent continuation task. Existing resource permissions still determine which materials are available.
...
- SnSworker is not a remote filesystem that shares every member's local directories.
