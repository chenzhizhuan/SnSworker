/**
 * SnSworker Electron CDP 解析 — 读 DevToolsActivePort，经 browser WebSocket 驱动 renderer。
 *
...
async function pickPageTarget(client, { page = 'main', vitePort = DEFAULT_VITE_PORT } = {}) {
  ...
  throw new Error(
    `未找到 SnSworker 主 renderer page（期望 localhost:${vitePort} 或 tabtin-file://app/index.html，非 overlay.html）。` +
      `当前 pages: ${pages.map((p) => p.url).join(', ') || '(none)'}`,
  )
}
