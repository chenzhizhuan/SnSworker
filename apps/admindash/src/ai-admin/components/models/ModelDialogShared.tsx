...
const IMAGE_INPUT_VIA_OPTIONS = [
  { value: 'base64', label: 'Base64', description: 'SnSworker 下载图片后内联发送，兼容性最好' },
...
export function ChatVisionPanel({
  ...
  return (
    <div className="grid grid-cols-2 gap-3 rounded border bg-muted/10 p-3">
      ...
      <Field label="流式回复" hint="关闭后上游生成完整答案，再由 SnSworker 一次性返回给客户端">
        <ToggleRow
      ...
      <Field
        label="上下文缓存"
        hint="表示 SnSworker 可主动使用的缓存方式；选“不支持”不会抹掉厂商响应中真实发生的自动缓存用量"
      >
        <select
          className="w-full rounded-md border px-3 py-2 text-body bg-background"
