...
<body>
  <div class="card">
    <h1>飞书授权已完成</h1>
    <p>正在返回智算方舟客户端。若未自动打开，请点击下方按钮。</p>
    <a class="btn" id="open" href="{safe_link}" data-href="{safe_link}">打开智算方舟</a>
  </div>
  <script>
    (function () {{
...
def _raise_feishu_list_permission_error(exc: FeishuAPIError, *, context: str) -> None:
    ...
    if (
        ...
        raise HttpError(
            ...
            "docx:document:readonly，并在智算方舟断开飞书后重新授权。",
        ) from exc
    ...
    try:
        ...
        if "99991679" in message or "board:whiteboard:node:read" in message:
            raise HttpError(
                ...
                "请开通 board:whiteboard:node:read，并在智算方舟断开后重新授权。",
            ) from exc
    ...
    try:
        ...
        if "drive:drive" in msg or "99991679" in msg or "search:docs:read" in msg:
            raise HttpError(
                ...
                "请在飞书开放平台开通 drive:drive:readonly，并在智算方舟断开后重新授权。",
            ) from exc
