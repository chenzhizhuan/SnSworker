...
class ReleaseAssetService:
    ...
    def _build_canonical_asset_file_name(
        ...
        *,
        ...
        if platform == "win":
            # 例：SnSworker-1.0.0-windows.exe
            return f"SnSworker-{version}-windows{extension}"

        if platform == "mac":
            if not arch:
                return ""
            if asset_type == "website_installer":
                # 例：SnSworker-1.0.0-arm64.dmg / SnSworker-1.0.0-x64.dmg
                return f"SnSworker-{version}-{arch}{extension}"
            # 自动更新 zip：保留 -mac 后缀，便于和官网 dmg 区分
            # 例：SnSworker-1.0.0-arm64-mac.zip
            if extension == ".zip":
                return f"SnSworker-{version}-{arch}-mac.zip"
            return f"SnSworker-{version}-{arch}{extension}"
