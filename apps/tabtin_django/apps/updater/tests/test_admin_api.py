...
""".strip()
    ...
    def test_staff_can_preview_manifest_template(self):
        ...
        release.file_url = "https://cdn.example.com/desktop-updates/stable/mac/x64/4.1.0/SnSworker-Setup-4.1.0.dmg"
    ...
    def test_superuser_can_upload_package_and_auto_generate_manifest(self):
        ...
        with patch("apps.updater.services.asset_service.get_updater_oss_service", return_value=fake_oss):
            intent_response = self.client.post(
                ...
                data=json.dumps(
                    {
                        "asset_type": "package",
                        "file_name": "SnSworker Setup 5.0.0.exe",
                        "file_size": 31457280,
                        "content_type": "application/octet-stream",
                    }
                ),
            ...
            self.assertTrue(intent_payload["object_key"].endswith("SnSworker-5.0.0-windows.exe"))
            self.assertEqual(intent_payload["expected_file_name"], "SnSworker-5.0.0-windows.exe")
        ...
        self.assertIn("SnSworker-5.0.0-windows.exe", manifest_text)
    ...
    def test_package_upload_rejects_platform_mismatched_installer(self):
        ...
        response = self.client.post(
            ...
            data=json.dumps(
                {
                    "asset_type": "package",
                    "file_name": "SnSworker-Preprod-5.0.2-arm64.dmg",
                    "file_size": 31457280,
    ...
    def test_superuser_can_upload_website_installer_without_touching_file_url(self):
        ...
        release.file_url = "https://cdn.example.com/desktop-updates/stable/mac/arm64/5.0.3/SnSworker-5.0.3-arm64-mac.zip"
        ...
        with patch("apps.updater.services.asset_service.get_updater_oss_service", return_value=fake_oss):
                ...
                data=json.dumps(
                    {
                        ...
                        "file_name": "SnSworker-5.0.3-arm64.dmg",
                        "file_size": 41943040,
                        "content_type": "application/x-apple-diskimage",
                    }
                ),
            ...
            self.assertEqual(intent_response.status_code, 200)
            intent_payload = intent_response.json()
            self.assertTrue(intent_payload["object_key"].endswith("SnSworker-5.0.3-arm64.dmg"))
        ...
        self.assertEqual(
            release.file_url,
            "https://cdn.example.com/desktop-updates/stable/mac/arm64/5.0.3/SnSworker-5.0.3-arm64-mac.zip",
        )
        self.assertFalse(payload["asset"]["manifest_generated"])
        self.assertEqual(payload["release"]["website_asset_name"], "SnSworker-5.0.3-arm64.dmg")
    ...
    def test_superuser_can_overwrite_same_object_key_without_duplicate_file_records(self):
        ...
        fake_oss = _FakeOSSService()
        # 托管上传会规范化为短文件名 SnSworker-{ver}-{arch}-mac.zip
        object_key = "desktop-updates/stable/mac/x64/5.1.0/SnSworker-5.1.0-x64-mac.zip"
        fake_oss.file_sizes[object_key] = 2048

        payload = {
            "asset_type": "package",
            "object_key": object_key,
            "file_name": "SnSworker-5.1.0-x64-mac.zip",
            "file_size": 2048,
            "content_type": "application/octet-stream",
