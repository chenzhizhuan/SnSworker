...
class TencentEmailService(EmailServiceBase):
    ...
    def send_verification_email(self, to_email: str, code: str) -> Dict[str, Any]:
        ...
        try:
            # 使用HTML模板渲染验证码邮件
            subject = f"【{self.config.get('company_name', 'SnSworker')}】邮箱验证码"
    ...
    def _render_verification_template(self, code: str) -> str:
        ...
        template_context = {
            'code': code,
            'company': self.config.get('company_name', 'SnSworker'),
    ...
    def _send_welcome_email(self, to_email: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """发送欢迎邮件"""
        subject = f"欢迎加入{self.config.get('company_name', 'SnSworker')}！"
        ...
        <body>
            ...
            <br>
            <p>祝好！</p>
            <p>{self.config.get('company_name', 'SnSworker')}团队</p>
        </body>
        </html>
        """
