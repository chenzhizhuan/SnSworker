...
def evaluate_cors_rules(
    ...
    *,
    ...
    return CorsCheckResult(
        ok=False,
        errors=best_failures or ["没有找到满足 SnSworker 直传契约的 CORS 规则"],
        warnings=collected_warnings,
        checked_rules=len(rules),
    )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--print-contract",
            action="store_true",
            help="输出 SnSworker 要求的 OSS CORS 契约，不访问 bucket",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not result.ok:
            raise CommandError("OSS bucket CORS 未满足 SnSworker 桌面端直传契约")
    ...
    def _emit(self, payload: dict[str, object], *, as_json: bool) -> None:
        ...
        if "allowed_origins" in payload:
            self.stdout.write("SnSworker OSS CORS contract")
