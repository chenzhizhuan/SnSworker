...
def _device_only_error_payload(
    ...
    if required_capability in _VIDEO_DEVICE_CAPABILITIES:
        if reason == "space_not_found":
            return {
                ...
                "error": (
                    ...
                    "请打开对应 Space 或 TabVideo 项目会话，并确保 Agent 已绑定 SnSworker Daemon 后重试。"
                ),
        ...
        if reason == "capability_mismatch":
            return {
                ...
                "error": (
                    ...
                    "请在执行设备安装 FFmpeg，启动或重启 SnSworker Daemon，然后刷新设备能力后重试。"
                ),
        ...
        if reason in {"device_offline", "runtime_ws_offline"}:
            return {
                ...
                "error": (
                    ...
                    "请启动 SnSworker Daemon 并确认设备已连接后重试。"
                ),
        ...
        if reason in {"no_control_device", "control_device_no_runtime"}:
            return {
                ...
                "error": (
                    ...
                    "请为当前 Agent 绑定 SnSworker Daemon 设备；如设备缺少 FFmpeg，请先安装后刷新能力。"
                ),
        ...
        if reason == "delivery_failed":
            return {
                "success": False,
                "error": f"{label}任务下发到后台设备失败，请确认 SnSworker Daemon 仍在线后重试。",
