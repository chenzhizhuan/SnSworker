...
def build_runtime_manifest_markdown(validation: RuntimeValidationResult) -> str:
    sections = [
        ...
        "## 0. 部署口径",
        "SnSworker Runtime 生产与本地部署统一采用 **8 组 worker + 单 beat**：`worker-critical`、`worker-default`、`worker-realtime`、`worker-search`、`worker-data-ai`、`worker-heavy`、`worker-tracker`、`worker-ai-background`，以及一个 Celery Beat 进程。ACK、deploy compose、systemd、supervisor 和本地脚本都应保持这个口径。",
        "## 1. 字段说明",
