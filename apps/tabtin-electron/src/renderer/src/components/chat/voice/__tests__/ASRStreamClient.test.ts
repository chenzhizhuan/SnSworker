...
describe('buildASRPayload', () => {
  ...
  it('should include hotwords in context', () => {
    const payload = buildASRPayload({ hotwords: ['SnSworker', 'Agent'] })
    expect(payload.context).toBeDefined()
    const parsed = JSON.parse(payload.context)
    expect(parsed.hotwords).toEqual([{ word: 'SnSworker' }, { word: 'Agent' }])
  })
...
describe('ASRStreamClient.start', () => {
  ...
  it('starts through gateway.request and activates the returned stream', async () => {
    ...
    await expect(client.start({ hotwords: ['SnSworker'] })).resolves.toBe('asr_test')

    expect(gateway.request).toHaveBeenCalledWith(
      'asr.stream.start',
      expect.objectContaining({
        provider: 'bytedance',
        context: expect.stringContaining('SnSworker'),
      }),
