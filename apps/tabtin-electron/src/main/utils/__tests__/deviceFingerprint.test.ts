...
let appDataRoot = ''
let currentUserDataDirName = 'SnSworker Dev'
...
describe('getDeviceIdentity（安装身份与重装恢复）', () => {
  beforeEach(() => {
    ...
    currentUserDataDirName = 'SnSworker Dev'
  ...
  it('首次安装生成 UUID，后续更新即使可读取硬件标识也不改变 fingerprint', () => {
    ...
    expect(readIdentity('SnSworker Dev').fingerprint).toBe(first.fingerprint)
  })

  it('已有历史随机 fingerprint 不会被硬件哈希覆盖', () => {
    writeIdentity('SnSworker Dev', { fingerprint: 'electron-legacy-install-id' })

    const identity = getDeviceIdentity()

    expect(identity.fingerprint).toBe('electron-legacy-install-id')
    expect(identity.machineKey).toHaveLength(32)
    expect(readIdentity('SnSworker Dev').fingerprint).toBe('electron-legacy-install-id')
  })

  it('删除 userData 后是新安装身份，但同 profile machineKey 保持不变供服务端恢复', () => {
    const first = getDeviceIdentity()
    fs.rmSync(path.join(appDataRoot, 'SnSworker Dev'), { recursive: true, force: true })
    _resetDeviceFingerprintCacheForTests()
  ...
  it('不同 profile 使用不同安装身份和 machineKey', () => {
    ...
    currentUserDataDirName = 'SnSworker'
  ...
  it('不从其它 profile 收集历史候选', () => {
    writeIdentity('SnSworker Preprod', { fingerprint: 'electron-current-preprod' })
    writeIdentity('SnSworker', {
    ...
    currentUserDataDirName = 'SnSworker Preprod'
  ...
  it('运行资料 revision 取服务端与本地较大值并只向前持久化', () => {
    ...
    expect(readIdentity('SnSworker Dev').runtime_profile_revision).toBe(6)
  })
})
