...
describe('ensureMcpRemoteClientName', () => {
  ...
  it('removes client_info when client_name is not SnSworker', async () => {
    ...
    ensureMcpRemoteClientName('https://mcp.stripe.com', 'SnSworker')
  ...
  it('clears tokens and client_info for the server url on uninstall', async () => {
    ...
    writeFileSync(join(dir, `${hash}_client_info.json`), JSON.stringify({ client_name: 'SnSworker' }))
  ...
  it('keeps client_info when client_name already matches', async () => {
    ...
    mkdirSync(dir, { recursive: true })
    const clientInfoPath = join(dir, `${hash}_client_info.json`)
    const payload = { client_name: 'SnSworker', client_id: 'oacli_ok', scope: 'mcp' }
