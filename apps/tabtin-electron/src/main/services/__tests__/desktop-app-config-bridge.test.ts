...
describe('resolveTabDesktopAppManifestRoot · v2.2 模块零扫尾（独立验收 P0-1）', () => {
  ...
  it('打包态（isPackaged=true + resourcesPath 有效）→ 返回 <resourcesPath>/app.asar.unpacked/packages/apps', () => {
    const r = resolveTabDesktopAppManifestRoot({
      isPackaged: true,
      resourcesPath: '/Applications/SnSworker.app/Contents/Resources',
    })
    expect(r).toBe('/Applications/SnSworker.app/Contents/Resources/app.asar.unpacked/packages/apps')
  })

  it('打包态 + Windows 路径 → 同样拼成 packages/apps', () => {
    const r = resolveTabDesktopAppManifestRoot({
      isPackaged: true,
      resourcesPath: 'C:\\Program Files\\SnSworker\\resources',
    })
