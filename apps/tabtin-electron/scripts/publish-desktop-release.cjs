...
const RELEASE_NOTE_FALLBACK_PREFIX = 'SnSworker Desktop release'
...
function buildCanonicalUploadFileName({ platform, arch, version, assetType = 'package', sourceFileName }) {
  ...
  if (platform === 'win') {
    return `SnSworker-${ver}-windows${ext}`
  }
  if (platform === 'mac') {
    if (assetType === 'website_installer') {
      return `SnSworker-${ver}-${normalizedArch}${ext}`
    }
    if (ext === '.zip') {
      return `SnSworker-${ver}-${normalizedArch}-mac.zip`
    }
    return `SnSworker-${ver}-${normalizedArch}${ext}`
  }
 ...
 *   例：`SnSworker-1.0.0-arm64.dmg`
 *       `SnSworker Preprod-1.0.0-preprod.1-arm64.dmg`
 *       `SnSworker Preprod-1.0.0-preprod.1-arm64-mac.zip`
 *
