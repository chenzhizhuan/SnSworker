...
function createMacFixtureDir() {
  ...
  const zipName = 'SnSworker-1.0.99-arm64-mac.zip'
  const dmgName = 'SnSworker-1.0.99-arm64.dmg'
  ...
  writeFileSync(
    ...
    'version: 1.0.99\nfiles:\n  - url: SnSworker-1.0.99-arm64-mac.zip\n'
  )
  return dir
}
    ...
    if (kind === 'asset_complete') {
      ...
      return jsonResponse({
        ...
        release: {
          ...
          channel: 'stable',
          file_url: isPackage
            ? 'https://cdn.example.com/desktop-updates/stable/mac/arm64/1.0.99/SnSworker-1.0.99-arm64-mac.zip'
            : 'https://cdn.example.com/desktop-updates/stable/mac/arm64/1.0.99/SnSworker-1.0.99-arm64-mac.zip',
          website_file_url: isWebsite
            ? 'https://cdn.example.com/desktop-updates/stable/mac/arm64/1.0.99/SnSworker-1.0.99-arm64.dmg'
            : '',
...
async function testMacStableDryRunRequiresDmg() {
  ...
  try {
    writeFileSync(path.join(dir, 'SnSworker-1.0.99-arm64-mac.zip'), Buffer.from('zip-only'))
    writeFileSync(path.join(dir, 'SnSworker-1.0.99-arm64-mac.zip.blockmap'), Buffer.from('bm'))
    let error = null
    try {
      await main([
        '--platform',
