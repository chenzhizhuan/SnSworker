...
```text
请在当前仓库运行 SnSworker 的 Android 项目：完成 Gradle sync，启动 Android Emulator
或连接已授权的真机，构建并安装 Debug APK，然后启动 SnSworker；如果发现构建错误，
请先修复或说明具体阻塞原因。
```
...
环境时，先在 PC 端启动 SnSworker，再使用 PC 端生成的二维码；Android 扫码后会自动
