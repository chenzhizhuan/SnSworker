...
command -v curl >/dev/null 2>&1 || fail \
  "curl is required to check SnSworker Server readiness."
...
printf 'Waiting for SnSworker Server readiness'
...
until curl -fsS --max-time 3 \
  ...
  if (( SECONDS >= deadline )); then
    ...
    docker compose \
      ...
      -f "${compose_file}" \
      ps
    fail "SnSworker Server did not become ready within ${timeout_seconds} seconds. Run ./status.sh or ./community logs for details."
  fi
  printf '.'
  sleep "${poll_seconds}"
done
