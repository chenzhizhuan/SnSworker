...
<p align="center">
  <img src="assets/readme/tabtin-collaboration-hero.png" alt="SnSworker collaboration between people and Agents" />
</p>

SnSworker is an open-source collaboration platform for people and Agents, built for individuals and teams that want to bring Agents into real work. We want to reduce the friction of working with Agents, so work completed by one person can be continued, reused, and reviewed by the team.

> **Try SnSworker:** [Visit the SnSworker website](https://tabtin.com/) to use the official hosted service without operating your own server.
>
...
SnSworker starts by asking one question: **Can work completed by one person become the next colleague's starting point?**

Teams already using AI deeply are SnSworker's first wedge, not its product boundary. SnSworker is for any individual or team that wants to bring Agents into real work.

## What individuals and teams can do with SnSworker
...
SnSworker provides collaboration applications for messaging, documents, data tables, and presentations. Agents can create and edit this content directly, and data and media collected by the browser can continue into the work applications.
...
SnSworker is currently in Public Preview and preparing its first complete public source release. Its core workflows are available to try, but components still differ in maturity and cross-platform consistency.
...
SnSworker is open-sourcing the actual product code, not a separate simplified demo.
...
1. **Official service**: for individuals and small teams that want to experience SnSworker without operating server infrastructure. Visit the [SnSworker website](https://tabtin.com/).
2. **Run from source locally**: for users and developers who want to try, debug, or extend SnSworker on their own computer. An Agent can prepare and start the environment, or you can run it manually.

### Run from source locally

There are two ways to run the SnSworker source locally: ask an Agent to prepare and start it, or run the scripts and commands yourself. Both use the same local development environment; neither is a Community distribution package or a production release build.
...
We welcome individuals, teams, and developers around the world who are using or exploring Agents to try SnSworker, report real problems, share working methods, or contribute code, documentation, Skills, work applications, and adapters.
...
Community Server listens only on the local machine by default and stores account, configuration, and business data in local Docker volumes. The Community client does not connect to SnSworker-maintainer Sentry or update services by default. Full diagnostic bundles may be uploaded only with explicit user consent. Data processing for the official service follows its published privacy policy.

## License and trademarks

SnSworker's public source is provided under [AGPL-3.0-only](LICENSE). Organizations whose use or distribution is incompatible with AGPL-3.0-only may contact Shanghai Mofan Technology Co., Ltd. at [contact@larchiveai.com](mailto:contact@larchiveai.com) about separate commercial licensing.

Third-party components remain subject to their respective licenses. See [THIRD_PARTY_NOTICES.en.md](THIRD_PARTY_NOTICES.en.md). The SnSworker name and marks are trademarks of the project maintainer. Forks may truthfully state that they are “based on SnSworker,” but must not impersonate an official release.
